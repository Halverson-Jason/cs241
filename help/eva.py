...
class Ball:
  ...
  def draw(self):
    ...
    arcade.draw_circle_filled(self.center.x + BALL_RADIUS , self.center.y + BALL_RADIUS,
  ...
  def restart(self):
    ...
    self.velocity.dx = random.uniform(2,MOVE_AMOUNT)
    self.velocity.dy = random.uniform(2,MOVE_AMOUNT)
...
class Paddle:
  ...
  def draw(self):
    ...
    arcade.draw_rectangle_filled(self.center.x, self.center.y,
                PADDLE_WIDTH,PADDLE_HEIGHT, arcade.color.DARK_SPRING_GREEN)
...
class Pong(arcade.Window):
  ...
  def check_bounce(self):
    ...
    if self.ball.center.y > SCREEN_HEIGHT - BALL_RADIUS*2 and self.ball.velocity.dy > 0:
      self.ball.bounce_vertical()
